...
const int default_height =
    static_cast<int>(default_width / default_aspect_ratio);
...
class Image {
  public:
    Image(double aspect_ratio, int width, int sample_per_pixel, int max_depth)
        : m_aspect_ratio(aspect_ratio), m_width(width),
          m_height(static_cast<int>(width / aspect_ratio)),
          m_sample_per_pixel(sample_per_pixel), m_max_depth(max_depth),
          m_num_pixels(m_width * m_height) {
        m_data = new Color[m_num_pixels];
    }

    Image()
        : m_aspect_ratio(default_aspect_ratio), m_width(default_width),
          m_height(default_height),
          m_sample_per_pixel(default_sample_per_pixel),
          m_max_depth(default_depth), m_num_pixels(m_width * m_height) {
        m_data = new Color[m_num_pixels];
    }

    Image(int width)
        : m_aspect_ratio(default_aspect_ratio), m_width(width),
    ...
    Color &operator[](int i) { return m_data[i]; }
    ...
    void write_png(const char *filename) {
        ...
        int num_component = 3;
        unsigned char  data[m_width * m_height * num_component];
        for (int i = 0, index = 0; i < m_num_pixels;
             i++, index += num_component) {
            double r = m_data[i].x();
            double g = m_data[i].y();
            double b = m_data[i].z();
            ...
            data[index] = static_cast<unsigned char>(256 * clamp(r, 0.0, 0.999));
            data[index + 1] = static_cast<unsigned char>(256 * clamp(g, 0.0, 0.999));
            data[index + 2] = static_cast<unsigned char>(256 * clamp(b, 0.0, 0.999));
        }
        stbi_write_png(filename, m_width, m_height, num_component, data, 0);
    }
    ...
    void write_img() {
        ...
        for (int i = 0; i < m_width * m_height; i += num_compoenent) {
            std::cerr << data[i] << " " << data[i + 1] << " " << data[i + 2]
                      << std::endl;
        }
        stbi_write_png(default_filename, m_width, m_height, 3, data, 0);
