...
class Vec3 {
  public:
    ...
    bool near_zero() const {
        return (std::fabs(m_components[0]) < close_to_zero) &&
               (std::fabs(m_components[1]) < close_to_zero) &&
               (std::fabs(m_components[2]) < close_to_zero);
    }
...
inline Vec3 operator/(double t, const Vec3 &v) { return 1 / t * v; }
